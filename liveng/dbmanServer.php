...
require_once "initServer.php";
 ...
 class dbops{
   ...
   function connect(){
    ...
    if($dbtype == 'mysql'){
    $conn = new mysqli($host,$unam,$upass,$database,$port);
    if($conn){    
     return $conn;
    }
    else return false;
   ...
   /*if($this->con == null){
    ...
    if($this->con){
      $fn = "$dbtype".'_query';
      if($dbtype == 'mysql')     
      $res = $this->con->query($qry,MYSQLI_USE_RESULT);
      if($dbtype == 'sqlite')
      $res = $fn($this->con, $qry);      
      if(!$res){
       if($dbtype == 'mysql') 
    ...
    $ern = "";
    if($dbtype == 'mysql'){$ern = $this->con->error;}
    //else{$ern = $dbtype.'_error'.();}     
     return $ern;
    }
   }
   ...
   function runDml($qry,$params){
   $dbtype = $this->getDBType();      
    if($this->con){
      $stmt = $this->con->prepare($qry);
      $v_t = "";
      foreach($params as $k=>$v){      
       if(is_string($v)){$v_t .= 's';}
       if(is_int($v)){$v_t .= 'i';}
       if(is_double($v)){$v_t .= 'd';}              
      }      
      $stmt->bind_param($v_t,extract($params,EXTR_PREFIX_SAME,'ilx'));
      $stmt->execute();
