<?php 
 ...
 set_time_limit(0);   
  ...
  $xlsFile = $rpt_section."_$report_title"."_$start_dtt.xls";
    ...
    $agent_ids = array(3317,3318,3733);
    $agent_lbl = 'agent';
   ...
   $start_row = 5;
   $col = 1;
   $headers = array('SN','Channels','Regions','Customer Codes','Customer Names','Address','Phone No.','Rep Name','Depot','Rep Visit','Used Last TLP','Selling Competition','How do u rate our Service','How do we serve you better','Time','Agent');   
   foreach($headers as $k => $v){
    ...
    $col++;
      ...
      $qry = "CALL load_outlets_feedback(?,?,?)";
      $stmt = $utils->db->con->prepare($qry);            
      $stmt->bind_param('iss',$theAgent,$start_dtt,$end_dtt);
      $stmt->execute();    
      $stmt->bind_result($channel,$region,$customer_code,$customer_name,$customer_location,$visit_freq,$tlp_use,$competition_sell,$service_rate,$serve_better,$cc_agent,$call_time,$customer_address,$customer_phone,$rep_name);      
       $sn = 0;  
       $giz = 0;   
       ...
       $sn++;          
        ...
        $headers = array('SN','Channels','Regions','Customer Codes','Customer Name','Address','Phone No.','Rep Name','Depot','Rep Visit','Used Last TLP','Selling Competition','How do u rate our Service','How do we serve you better','Time','Agent');
        $giz += 1;
        $rowNum = $start_row+$giz;         
        $details = array($sn,$channel,$region,$customer_code,$customer_name,$customer_address,$customer_phone,$rep_name,$customer_location,$visit_freq,$tlp_use,$competition_sell,$service_rate,$serve_better,$call_time,$cc_agent
                         );
